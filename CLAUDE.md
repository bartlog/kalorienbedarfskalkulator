...
- `js/tipps-daten.js` — static content data (`window.KBR.tipps.ABSCHNITTE`, array of `{id, titel, spalten, zeilen}`) for the Tipps tab: curated fat-loss/recomposition guidance (nutrition, food/drink, restaurant behavior, training, supplements), each row tagged `Top-Auswahl`/`Gute Auswahl`/`OK`/`Selten`/`Vermeiden` (or `Phase` for the restaurant table, which has no rating column). This is a manually-updated snapshot of a private Google Sheet the user maintains — the sheet's URL is deliberately *not* in this repo (see memory `reference_tipps_spreadsheet` for the fileId and the regeneration workflow); when the user says the sheet changed, re-fetch it and regenerate this file, don't try to guess at edits.
- `js/ui.js` — the only file touching the DOM: tab switching, conditional field visibility, reading the form into the pipeline's input shape, rendering results (including the inline-SVG bar/band graphics), rendering the Tipps accordion/filter UI (`renderTipps`), wiring save/delete/print buttons.
